...
$password = password_hash('xgWv42hCDv4muVxbmx', PASSWORD_DEFAULT);
		...
		<div class="container container--mail">
			<div class="container__inner">
				...
				<div id="formWrap" class="ta_center">
					<?php if ($empty_flag == 1) { ?>
						<div>
							<h3 class="mb_02 form_ttl">お問い合わせ内容確認画面</h3>
...
<?php
		...
		function postToMail($arr)
		{
			...
			$resArray = '';
			foreach ($arr as $key => $val) {
				if (($key == "申込みのきっかけ記入欄" && $_POST["申込みのきっかけ"] != "その他")) {
					//サンクス文面とイベント名は表示しない
				} else {
					$out = '';
					if (is_array($val)) {
...
?>
	...
	<body>
		...
		<?php endif;?>
		<h1 class="mt_05 fs_24 ta_center">お問い合わせCSVダウンロード認証画面</h1>
		<div id="login_form">
